...
const starts = async (haruka = new WAConnection()) => {
    ...
    haruka.browserDescription = [ 'Sean-Bot', 'Chrome', '3.0' ]
	CFonts.say('Haruka', {
		font: 'block',
    	color: ['#ff9c00'],
    	align: 'center',
	...
	haruka.on('group-participants-update', async (anu) => {
		...
		try {
						...
						if (anu.action == 'add' && !num.includes(haruka.user.jid)) {
							...
							try{
							await sendButLoc(mdata.id, `Goodbye @${num.split('@')[0]}\n⌯ָ   ֙Leave from group:\n${mdata.subject}` + '\n' + lang.leave(), `Leave Message By ${ownername}`,goodbye, [{"buttonId": `.bye`,"buttonText": {"displayText": "Bye 🤲"},"type": "RESPONSE"}], {contextInfo: { mentionedJid: [num]}})
							} catch {
								await sendButLoc(mdata.id, `Goodbye @${num.split('@')[0]}\n⌯ָ   ֙Leave from group:\n${mdata.subject}` + '\n' + lang.leave(), `Leave Message By ${ownername}`,ppmem2, [{"buttonId": `.bye`,"buttonText": {"displayText": "Bye 🤲"},"type": "RESPONSE"}], {contextInfo: { mentionedJid: [num]}})
							}
...
if (antical === false) return
        ...
        haruka.sendMessage(callerId, { displayname: `${global.ownername}`, vcard: vcard}, MessageType.contact, {contextInfo: { externalAdReply:{title: `Creator ${setting.botname}`,body:"",previewType:"PHOTO",thumbnail: global.thumbnail ,sourceUrl:`https://wa.me/${global.ownernumber}?text=Assalamualaikum kak, buka blokir saya`}}})
        await sleep(5000)
